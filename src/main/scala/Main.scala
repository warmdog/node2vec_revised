...
object Main {
  ...
  case class Params(iter: Int = 10,
                    ...
                    input: String = null,
                    output: String = null,
                    usersFiles : String="select phone_no from xyqb.user ",
  ...
  def main(args: Array[String]) :Unit= {
    ...
    parser.parse(args, defaultParams).map { param =>
      val spark = SparkSession.builder().appName("Node2Vec").getOrCreate()
      val context: SparkContext = spark.sparkContext
      
      Node2vec.setup(spark,context, param)
