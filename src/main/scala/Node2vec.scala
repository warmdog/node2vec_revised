...
import graph.{EdgeAttr, NodeAttr}
...
object Node2vec extends Serializable {
  ...
  def setup(sparkSession: SparkSession,context: SparkContext, param: Main.Params): this.type = {
  ...
  def load(): this.type = {
    ...
    indexedNodes = indexedNodes.join(users).filter(x  =>x._2._2==1).map(x =>(x._1,x._2._1)).filter(_._2.neighbors.length>0).repartition(2000).cache

    indexedEdges = indexedNodes.flatMap { case (srcId, clickNode) =>
      clickNode.neighbors.map { case (dstId, weight) =>
          Edge(srcId, dstId, EdgeAttr())
      }
    }.repartition(2000).cache
    println(indexedEdges.count())
    this
  }
  ...
  def indexingGraph(rawTripletPath: String): RDD[(Long, Long, Double)] = {
    val rawEdges = context.textFile(rawTripletPath).map { triplet =>
      val parts = triplet.split("\\s+")
      //Try(parts.last.toDouble).getOrElse(1.0)
      Try {
        (parts.head, parts(1), Try(parts(2).toDouble).getOrElse(1.0))
      }.getOrElse(null)
    }.filter(x =>(x._1!=x._2)&&x!=null)
  ...
  def getUsers() :RDD[(Long,Int)]= {
    val regex = "[0-9]".r
    sparkSession.sql(config.usersFiles).rdd.map(x =>{
      val s=x.toString()
